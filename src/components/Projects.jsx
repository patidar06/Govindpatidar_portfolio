        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((proj, idx) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="p-8 rounded-3xl bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 hover:border-cyan-500/50 dark:hover:border-cyan-500/40 shadow-xl shadow-slate-200/50 dark:shadow-none backdrop-blur-md flex flex-col justify-between transition-all duration-300 group"
            >
              <div>
                {/* Header info */}
                ...
                {/* Title & Desc */}
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mt-4 tracking-tight group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                  {proj.title}
                </h3>
              ...
              <div className="mt-8 pt-6 border-t border-slate-200/80 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex flex-wrap gap-2">
                  {proj.tags.map((tag) => (
                    <span 
                      key={tag} 
                      className="text-xs px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200/60 dark:border-white/5 text-slate-700 dark:text-slate-300 font-mono"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 text-white dark:bg-cyan-500 dark:text-black font-semibold text-xs hover:bg-cyan-600 dark:hover:bg-cyan-400 transition shadow-sm"
                >
